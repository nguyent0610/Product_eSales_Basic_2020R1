...
var HQ = {
    ...
    grid: {
        ...
        checkValidateEdit: function (grd, e, keys, isCheckSpecialChar) {
            if (keys.indexOf(e.field) != -1) {
                ...
                if (HQ.grid.checkDuplicate(grd, e, keys)) {
                    ...
                    var text = "";                    
                    for(var i = 0;i<keys.length;i++){
                        if(grd.columns.length > 0){
                            for(var j = 0;j<grd.columns.length; j++){
                                if(grd.columns[j].dataIndex == keys[i]){
                                    if(grd.columns[j].xtype == "datecolumn"){
                                        text += grd.columns[j].text + "(" + Ext.Date.format(e.value, e.column.format)  +")" + " - ";
                                    }
                                    else{
                                        var dataSelmodel = grd.selModel.selected.items[0].data[keys[i]];    
                                        var dataValue = grd.columns[j].field == undefined ? "" : grd.columns[j].field.field.value;
                                        var dataRawValue = grd.columns[j].field == undefined ? "" : grd.columns[j].field.field.rawValue;        
                                        text += grd.columns[j].text + "(" + (grd.columns[j].field == undefined ? (dataSelmodel) : (dataRawValue == "" ? ((Ext.isEmpty(dataValue) == true) ? (grd.columns[j].field.field.xtype == "combobox" ? grd.columns[j].initialConfig.renderer(dataSelmodel) : "") : dataValue) : dataRawValue)) + ")" + " - ";
                                    }
                                    break;
                                }
                            }
        ...
        headerclickCheckColumn: function (idCol, id, grd, str) {
            if (idCol.events.headerclick.name == undefined) {
                idCol.addListener('headerclick', function (record, item) {
                    if (item.id === "chkActive") {
                        var clazz = $('#chkActive_All').attr('class');
                        var checked = false;

                        if (clazz.indexOf('checkbox-header-checked') != -1) {
                            checked = false;
                            $(id).removeClass('checkbox-header-checked')
                        } else {
                            checked = true;
                            $(id).addClass('checkbox-header-checked')
                        }
    ...
    common: {
        ...
        changeData: function (isChange, screenNbr) {
            if (parent.App['tab' + screenNbr] != undefined)
                if (isChange)
                    parent.App['tab' + screenNbr].setTitle(HQ.common.getLang(screenNbr) + '*');
                else parent.App['tab' + screenNbr].setTitle(HQ.common.getLang(screenNbr));
        },
...
Ext.define("HQRowNumberer", {
    override: "Ext.grid.column.RowNumberer",
    width: 35
});
Ext.define("HQCheckboxModel", {
    override: "Ext.selection.CheckboxModel",
    headerWidth:30
});
Ext.define('HQFieldBase', {
